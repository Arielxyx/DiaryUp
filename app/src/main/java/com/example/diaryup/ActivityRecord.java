			...
			switch (v.getId()) {
				...
				case R.id.btn_record:
					...
					if (isRecording == 0) {
						...
						if(isPlaying==1){
							...
							Toast.makeText(ActivityRecord.this, "试听结束，重新开始录音",
				...
				case R.id.bt_save:
					...
					if (FilePath == null) {
						...
						if(isRecording==1){
							Toast.makeText(ActivityRecord.this, "请录音完毕后保存",
									Toast.LENGTH_LONG).show();
						}else if(isPlaying==1){
//							tv_recordTime.setText("00:00:00");
							isPlaying = 0;
				...
				case R.id.bt_back:
					// 返回前将录音的文件删除
					if(isRecording==1){ //如果正在录音要先停止录音再退出
						// 将按钮换成开始录音
						isRecording = 0;
						btn_record.setBackgroundResource(R.drawable.tabbar_record_start);
						mRecorder.stop();
					...
					if(isPlaying==1){//如果正在播放要先停止播放再退出
						isPlaying = 0;
						mPlayer.stop();
						mPlayer.release();
						mPlayer = null;
